...
def sync():
    ...
    for course in courses:
        for assignment in tqdm(
            canvas_user.get_assignments(course), desc=f"Syncing {course.name}"
        ):
            if "due_at_date" not in assignment.__dict__:
                tqdm.write(
                    f"[INFO] Skipping {assignment.name} because it has no due date"
                )
                continue
            try:
                ...
                if id in blacklist:
                    tqdm.write(
                        f"[INFO] Skipping {assignment.name} because it is blacklisted"
                    )
                    continue
...
Canvas Assignment ID: {id}"""
                start_date = (
                    assignment.unlock_at_date
                    if "unlock_at_date" in assignment.__dict__ is not None
                    else assignment.created_at_date
                )
                ...
                if "Lecture Attendance" in name:
                    ...
                    start_date = start_date.replace(
                        hour=21, minute=45
                    )  # fix all the times to be after lecture
                ...
                if "submitted_at" in submission.__dict__:
                    if submission.submitted_at is not None:
                        ...
                        tqdm.write(
                            f"[INFO] Skipping {name} because it is already submitted"
                        )
                ...
                with tasks[id] if id in tasks else ReclaimTask() as task:
                    if id not in tasks:
                        tqdm.write(f"[INFO] Adding {name}")
                        if task.name != name:
                            tqdm.write(f"[INFO] Updating {task.name} to {name}")
                        if task.start_date != start_date:
                            tqdm.write(
                                f"[INFO] Updating {task.name} start date from {task.start_date} to {start_date}"
                            )
                        if task.due_date != due_date:
                            tqdm.write(
                                f"[INFO] Updating {task.name} due date from {task.due_date} to {due_date}"
                            )
                    task.name = name
                    ...
                    task.duration = (
                        task.duration
                        if id in tasks and not FORCE_UPDATE_DURATION
                        else duration
                    )
    ...
    for id, task in tasks.items():
        if id not in assignments or id in blacklist:
            print(f"[INFO] Deleting {task.name}")
